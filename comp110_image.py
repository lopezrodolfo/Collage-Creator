...
class Pixel:
    ...
    def __str__(self):
        ...
        return "Pixel at (%d, %d) with red=%d, green=%d, blue=%d" % (
            self.__x,
            self.__y,
            rgb[0],
            rgb[1],
...
class Picture:
    ...
    def __init__(self, width=100, height=100, title=None, pic=None, filename=None):
        ...
        if pic is not None:
            ...
            self.__pixels = [
                [Pixel(pic.get_pixel(x, y), x, y) for x in range(self.__width)]
            ...
            def get_image_pixel(img, x, y):
                ...
                if img.mode == "RGB":
                    ...
                    print(img.mode)
                    raise RuntimeError(
                        "Image in %s has unsupported mode: %s" % (filename, img.mode)
                    )

                return Pixel(color, x, y)

            self.__pixels = [
                [get_image_pixel(image, x, y) for x in range(self.__width)]
                for y in range(self.__height)
            ]
            self.__title = title
            ...
            self.__pixels = [
                [Pixel(Black, x, y) for x in range(width)] for y in range(height)
            ]
            self.__title = title
    ...
    def show(self):
        ...
        canvas.create_image(
            (self.__width / 2, self.__height / 2), image=img, state="normal"
        )
    ...
    def __str__(self):
        return "A picture with width = %d and height = %d" % (
            self.__width,
            self.__height,
        )
    ...
    def __eq__(self, other):
        ...
        objects though.
        """
        if self.__width != other.get_width() or self.__height != other.get_height():
            return False
